...
<!--    :isGetListByQuery="true"-->
    ...
    <template slot="search">
        <template v-if="searchDateRangShow">
          ...
          <div class="el-form-item el-form-item--medium">
            <label class="el-form-item__label">审核时间:</label>
            <div class="el-form-item__content">
              <el-date-picker
                      size="small"
...
export default {
  ...
  methods: {
    ...
    sortMethod(val,arr){
      ...
      }else if(arr[0].key=='initiateTime'){
        this.outSiderQuery = {
          sort : sortType,
          order : 'r2.createTime'
        }
    ...
    handleEdit(row){
      ...
      this.$router.push({
        path: `/leadertask/auditCenter/reviewed/detail`,
        query : {
          id: row.id,
          commit: row.commit,
    ...
    resetSearchFun() {
      this.searchUpDate = []
      this.searchSetDate = []
      this.getListInterface.params = { status:50};
