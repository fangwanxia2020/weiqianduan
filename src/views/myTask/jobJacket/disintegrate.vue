...
<script>
  ...
  import {breakTask} from "@/api/myTask/jobJacket";
  ...
  export default {
    ...
    methods: {
      initFolderList(){
        getListByAccess({
          ...
          this.folderList = response.data.map((v) => {
            if(v.name == '日度计划') this.editData.createFolderId = v.id
            return {
              dictValue: v.id,
              dictLabel: v.name,
            };
      ...
      async init(){
        const {id,jobName} = this.$route.query
     ...
     handleSubmit(){
       ...
       this.formValidate(
        ...
        let arr = [this.checked ? this.editData1.show : this.editData2.show , this.editData3.show]
     ...
     async getSespTaskType(){
      ...
      if (res.code == 200 && Array.isArray(res.data)) {
        this.fgflList = res.data.map((v,index) => {
          if(index == 0) this.editData.tackTypeId = v.id
          return {
            dictValue: v.id,
            dictLabel: v.name,
          };
     ...
     back() {
        let path = "/leadertask/myTask/jobJacket";
