            ...
            <div class="design-form-wrap__ctnwrap">
              ...
              { label: '名称', prop: 'name' },
              {
                ...
                <el-row class="tip-sty">
                    <el-col :span='24'> {{dayName}}任务将于{{days}}{{haoNumber}}重复</el-col>
                </el-row>
                <el-row class="tip-sty">
...
<script>
  ...
  export default {
     ...
     getData(val){
      ...
      handleSubmit() {
        ...
        this.formValidate(this.editData, this.editDataRules, this.editDataErrorMsg).then(formDataOk => {
          ...
          const getTimeTaskStartTime = new Date(this.editData.cycleStartDate.substring(0, 11) + ' ' + this.editData.cycleStartTime + ':00').getTime();
          const getTimeTaskEndTime = new Date(this.editData.cycleEndDate.substring(0, 11) + ' ' + this.editData.cycleEndTime + ':00').getTime();
          const periodicEndTime = new Date(this.editData.repeatDate.substring(0, 11)+' 23:59:59').getTime();
      ...
      getTaskCount() {
        ...
        // if(!postData.cycleStartDate||!postData.cycleEndDate||!postData.cycleStartTime||!postData.cycleEndTime||!postData.endRepeat||!postData.nums||!postData.cycleFrequency){
      ...
      cycleEndDateChange() {
        this.pickerOptions2={
          disabledDate: (time) => {
            if (!this.editData.cycleEndDate) return false;
            if (this.editData.cycleEndDate) {
              let scoreCycle = this.editData.cycleEndDate
        ...
        //假设修改任务结束日期，晚于结束重复的日期，结束重复的日期也会统一改成相同的日期。

        //任务结束日期
        var strtime = this.editData.cycleEndDate;
        ...
        if (time1 > time2) {
          this.editData.repeatDate = this.editData.cycleEndDate
        }
      ...
      changecycleStartDate() {
        ...
        if (this.editData.generationDate == 2) {
          this.$nextTick(() => {
            ...
            this.days = '每月' + this.editData.cycleStartDate.substring(7, 9) + '号'
    ...
    return new Date(dateString) - 0
      ...
      cycleEndTimeChange() {
        var strtime = this.editData.cycleEndDate.substring(0, 11) + ' ' + this.editData.cycleEndTime + ':00';
        var date = new Date(strtime.replace(/-/g, '/'));
        let time1 = date.getTime();
